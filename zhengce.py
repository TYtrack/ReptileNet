#https://news.bjx.com.cn/list?catid=827&page=1
...
# 东北
def search_url_8(url_main,filefolder,pre_url):
    headers = {
    ...
    if response.status_code == 200:
        ...
        for j in list_title:
            try:
                ...
                get_one_page_8(temp_url,filefolder)
            except Exception:
                print("false")
            

def get_one_page_8(url, filefolder):
    try:
        ...
        if response.status_code == 200:
            ...
            write_txt(filefolder, get_biaoti_8(soup), get_full_8(soup))
        response.close()
...
def write_txt(filefolder,biaoti,full_text):
    biaoti=biaoti.strip()
    f=open("C:/Users/HUST/Desktop/中孚项目/政策/"+biaoti+".txt","w+",encoding='utf-8')
    f.write(biaoti+"\n")
    f.write(full_text)
    f.close()
...
for k in range(300):
    search_url_8("https://shupeidian.bjx.com.cn/NewsList?id=100&page="+str(k),"政策","https://news.bjx.com.cn/")

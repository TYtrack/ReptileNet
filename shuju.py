...
# 东北
def search_url_12(url_main,filefolder,pre_url):
    headers = {
    ...
    if response.status_code == 200:
        ...
        for j in list_title:
            try:
                ...
                get_one_page_12(temp_url,filefolder)
            except Exception:
                print("false")
            

def get_one_page_12(url, filefolder):
    try:
        ...
        if response.status_code == 200:
            ...
            write_txt(filefolder, get_biaoti_12(soup), get_full_12(soup))
        response.close()
...
def write_txt(filefolder,biaoti,full_text):
    biaoti=biaoti.strip()
    f=open("C:/Users/HUST/Desktop/中孚项目/数据/"+biaoti+".txt","w+",encoding='utf-8')
    f.write(biaoti+"\n")
    f.write(full_text)
    f.close()
...
for k in range(300):
    search_url_12("https://shupeidian.bjx.com.cn/NewsList?id=111&page="+str(k),"数据","https://news.bjx.com.cn/")
